...
routerProductos.get('/:id', async (req, res, next) => {
    ...
    try {
        ...
        if (error.code === "ENOENT") { res.send({ error: "Error: Producto no encontrado" }) }
        else {
            return next(error)
        }
    }
...
routerProductos.put("/:id", (req, res, next) => {
    ...
    response = contenedor.updateById(updateProduct, id)
        .then(res.send({ message: `El producto con id ${id} se actualizó correctamente` }))
        .catch(res.send({ message: 'El id es invalido' }))
})
